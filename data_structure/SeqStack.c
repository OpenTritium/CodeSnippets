...
sqStack CreateStack(void) {
  const u_int8_t STACK_SIZE = MAX_ELEMENT_QUANTITY * sizeof(ElementType);
 ...
 * @return false 栈不为空
 */
bool IsNull(const pSqStack pStk) { return pStk->size == 0; }
 ...
 */
ElementType GetTop(const pSqStack pStk) { return *(pStk->top - 1); }
